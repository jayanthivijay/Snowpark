...
def create_promocode_dim(all_sales_df,session)-> None:
    ...
    #fetch existing product dim records.
    existing_promo_code_dim_df = session.sql("select promotion_code from sales_dwh.consumption.promo_code_dim")


    promo_code_dim_df = promo_code_dim_df.join(existing_promo_code_dim_df,["promotion_code"],join_type='leftanti')


    promo_code_dim_df = promo_code_dim_df.selectExpr("sales_dwh.consumption.promo_code_dim_seq.nextval as promo_code_id_pk","promotion_code", "isActive") 
...
def main():
    ...
    promo_code_dim_df = session.sql("select promo_code_id_pk,promotion_code from sales_dwh.consumption.PROMO_CODE_DIM")
    ...
    all_sales_df = all_sales_df.join(promo_code_dim_df, ["promotion_code"],join_type='inner')
    all_sales_df = all_sales_df.join(region_dim_df, ["country", "region"],join_type='inner')
    
    processed_order_df = session.sql("select order_code as order_id from sales_dwh.consumption.sales_fact")
    all_sales_df = all_sales_df.join(processed_order_df, processed_order_df['order_id']==all_sales_df['order_id'], join_type='leftanti') 
